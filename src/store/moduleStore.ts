...
import type { ModuleState, ModuleConfig, CharacterSetOption, PlugboardModuleConfig } from './types';
...
export const useModuleStore = create<ModuleStoreState>((set, get) => ({
  ...
  // Add these new methods for plugboard functionality
  connectPlugboardPair: (moduleId, from, to) => {
    set(state => {
      const modules = [...state.modules];
      const moduleIndex = modules.findIndex(m => m.id === moduleId && m.type === "plugboard");
      ...
      Object.keys(newMapping).forEach(key => {
        if (newMapping[key] === from || newMapping[key] === to || key === from || key === to) {
